...
public class MyDraughtsPlayerTest extends DraughtsPlayer {
    ...
    int evaluate(DraughtsState state) {
        ...
        int result = 0;
        result += evalPieceCount(state) * 10;
        result += evalAttackCount(state);
        result += evalGoodPosition(state);
        result += evalTempiCount(state);
        return result;
    }

    private int evalPieceCount(DraughtsState state) {
        int[] pieces = state.getPieces();
        int totalScore = 0;
        for (int i = 0; i < pieces.length; i++) {
            if (pieces[i] == 1) { // White pawn
                totalScore += 1;
            } else if (pieces[i] == 2) { // Black pawn
                totalScore -= 1;
            } else if (pieces[i] == 3) { // White king
                totalScore += 3;
            } else if (pieces[i] == 4) { // Black king
                totalScore -= 3;
            }
        }
        return totalScore;
    ...
    private int evalAttackCount(DraughtsState state) {
        int attackCount = 0;
        List<Move> moves = state.getMoves();
        if (state.isWhiteToMove()) { // White attack moves
            for (Move move : moves) {
                if (move.isCapture()) {
                    attackCount++;
                }
            }
        } else { // Black attack moves
            for (Move move : moves) {
                if (move.isCapture()) {
                    attackCount--;
                }
    ...
    private int evalTempiCount(DraughtsState state) {
        int totalScore = 0;
        int[] pieces = state.getPieces();
        for (int i = 0; i < pieces.length; i++) {
            if (i < 6) { // Top row
            ...
            } else if (6 <= i && i < 11) { // 2nd top row
                if (pieces[i] == 1 | pieces[i] == 3) { // If white
                    totalScore += 8;
                } else { // Else black
                    totalScore -= 2;
                }
            } else if (16 <= i && i < 21) { // 3rd top row
                if (pieces[i] == 1 | pieces[i] == 3) {
                    totalScore += 7;
                } else {
                    totalScore -= 3;
                }
            } else if (21 <= i && i < 26) { // 4th top row
                if (pieces[i] == 1 | pieces[i] == 3) {
                    totalScore += 6;
                } else {
                    totalScore -= 4;
                }
            } else if (26 <= i && i < 31) { // 5th top row
                if (pieces[i] == 1 | pieces[i] == 3) {
                    totalScore += 5;
                } else {
                    totalScore -= 5;
                }
            } else if (31 <= i && i < 36) { // 6th top row
                if (pieces[i] == 1 | pieces[i] == 3) {
                    totalScore += 4;
                } else {
                    totalScore -= 6;
                }
            } else if (36 <= i && i < 41) { // 7th top row
                if (pieces[i] == 1 | pieces[i] == 3) {
                    totalScore += 3;
                } else {
                    totalScore -= 7;
                }
            } else if (41 <= i && i < 46) { // 8th top row
                if (pieces[i] == 1 | pieces[i] == 3) {
                    totalScore += 2;
                } else {
                    totalScore -= 8;
                }
            } else if (46 <= i && i < pieces.length) { // Bottom row
                if (pieces[i] == 2 | pieces[i] == 4) { // Black
                    totalScore -= 9;
                }
            }
        }
    ...
    private int evalGoodPosition(DraughtsState state) {
        int totalScore = 0;
        int pieces[] = state.getPieces();
        for (int i = 0; i < pieces.length; i++) {
            if ("middle".equals(getPosition(i))) { // If piece in middle
                if (pieces[i] == 1 || pieces[i] == 3) { // If white
                    totalScore++;
                } else { // Else black
                    totalScore--;
                }
            } else if ("top".equals(getPosition(i))) { // If piece at top
                if (pieces[i] == 2 || pieces[i] == 4) { // Black at home
                    totalScore--;
                }
            } else if ("bottom".equals(getPosition(i))) { // If piece at bottom
                if (pieces[i] == 1 || pieces[i] == 3) { // White at home
                    totalScore++;
                }
            }
        }
